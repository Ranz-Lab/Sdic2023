...
'''
[usage] sh seq_pairwise_alinment.py  infile s1 s2 outfile       ###What is infile, s1, s2, and outfile?
...
def seq_pairwise_alinment(infile,seqcol1,seqcol2,gap_penalties,outfile,sep):
    for eachline in infile:
        ...
        seq2 = tmp[seqcol2]
        ###s1, s2, sc, i1, i2
        alin = pairwise2.align.globalxs(seq1,seq2,0,0)
...
###        out = sep.join(out_list)
        if len(alin)==1:
            ...
            score=[]
            for i in range(0,len(alin)-1):
                score.append(alin[i][2])
                maxS_index = score.index(max(score))
                max_alin = alin[maxS_index]
        tmp.append(max_alin)
